...
class EditorWidget(QsciScintilla):
    ...
    def update_theme(self, theme=None):
        ...
        try:
            ...
            # Base colors
            self.setPaper(QColor(colors.get('editor_background', '#151a20')))
            self.setColor(QColor(colors.get('editor_foreground', '#e6eef6')))
            
            # Selection and caret
            self.setSelectionBackgroundColor(QColor(colors.get('editor_selection', 'rgba(14, 165, 233, 0.2)')))
            self.setSelectionForegroundColor(QColor(colors.get('foreground', '#ffffff')))
            self.setCaretForegroundColor(QColor(colors.get('foreground', '#ffffff')))
            
            # Margins
            self.setMarginsBackgroundColor(QColor(colors.get('editor_background', '#151a20')))
            ...
            try:
                self.setPaper(QColor('#151a20'))
                self.setColor(QColor('#e6eef6'))
            except Exception:
                pass
            ...
            try:
                ...
                try:
                    if ext == 'py' or ext == 'pyw':
                        from .lexers.python_lexer import apply_python_lexer
                        # build a colors dict from theme if present
                        try:
                            from core.theme_manager import ThemeManager
                            tm = ThemeManager()
                            try:
                            ...
                            theme_colors = {}
                        # prefer editor font for lexer tokens
                        qfont = None
                        try:
                            qfont = QFont(self.settings.editor.get('font_family', 'Consolas'), int(self.settings.editor.get('font_size', 12)))
                        except Exception:
                            qfont = None
                        try:
                            apply_python_lexer(lexer, theme_colors, qfont)
                        except Exception:
                            pass
                except Exception:
                    pass
